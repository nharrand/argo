...
reportsDir = "~/Documents/depswap/test-bench/results/"
...
dataset <- data.frame(`Parser`=character(),`Category`=character(),`File`=character(),`Result`=character())
sapply(dataset, typeof)

#read_csv(paste(reportsDir, "cookjson_correct_results.csv", sep = ""), col_names = T, col_types = cols())
...
```{r toto}
p <- dataset %>% select(File,Parser,Category, Result) %>% 
  mutate(Result = ifelse(Category == "errored" & (Result == "PARSE_EXCEPTION" | Result == "NULL_OBJECT"), "EXPECTED", 
                         ifelse(Category == "errored" & Result == "UNEXPECTED_OBJECT", "SILENT", 
                                ifelse(Category == "errored","ERROR",
                                       ifelse(Category == "correct" & (Result == "OK" | Result == "EQUIVALENT_OBJECT"), "EXPECTED",
...
p
ggsave("bench_simple_errored.pdf", p, device = "pdf",units = "cm", width = 21, height = 120)
p <- dataset %>% select(File,Parser,Category, Result) %>% 
  mutate(Result = ifelse(Category == "errored" & (Result == "PARSE_EXCEPTION" | Result == "NULL_OBJECT"), "EXPECTED", 
                         ifelse(Category == "errored" & Result == "UNEXPECTED_OBJECT", "SILENT", 
                                ifelse(Category == "errored","ERROR",
  ...
  ggplot(aes(x = Parser, y = File, fill=r)) +
      ...
      scale_y_discrete(drop = FALSE) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
p
ggsave("bench_simple_correct.pdf", p, device = "pdf",units = "cm", width = 21, height = 120)
```
...
pw_dist_violine_and_boxcolor <- ggplot(aes(y = `Pairwise distance`, x = `Corpus`, fill = `Corpus`), data=disbis) + 
  geom_violin() +
  geom_boxplot(width=0.1) +
  theme_bw() + 
  scale_fill_manual(values=c("#AA2222", "#22AA22")) +
  theme(text = element_text(size=16)) +
  scale_y_continuous(limits = c(0,1)) +
  labs(y = "Pairwise behavioral distance", x = element_blank()) + 
  coord_flip()

pw_dist_violine_and_boxcolor
ggsave("pairwise_distance_distributions_v_b_color.svg", pw_dist_violine_and_boxcolor,device="svg", height = 2, width = 6,  units = c("in"))
```
